...
The feedback report mirrors the removal's: DISLIKE out, LIKE back.

The one field that does **not** survive is the autogenerated `id`. Room reassigns
it, so a restored row comes back as 2, then 3, and so on — measured on the device
by reading the row back, not assumed. Nothing depends on the old value: the id is
not persisted anywhere else, `removeFavorite` re-reads the entity it is handed,
and the `DiffCallback` treating a restored row as a new item is the right
animation for a restore in any case. The position guarantee is carried by
`addedAt` alone, which is the field that was verified.
