...
- **Draft PR by default.** Every change lands via a pull request, opened as draft.
- **Completing a PR.** An agent may mark a PR ready and merge it once the task has passed the required review and validation gates **and** the owner has authorized completion of that slice. Before merging, confirm all four: the reviewed HEAD is unchanged, CI is green, the PR is mergeable and `CLEAN`, and the diff holds no unexpected files or scope expansion. Merge with the repository's normal squash convention unless told otherwise, and **never enable auto-merge** unless it was explicitly requested. After merging: fast-forward local `main` only, verify the merged tree is identical to the reviewed PR head's tree, then delete the remote and local feature branches. If validation fails, the HEAD moves, a conflict appears or the scope changes — stop and report instead of merging.
