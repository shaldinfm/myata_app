...
class FavoritesViewModel(application: Application) : AndroidViewModel(application) {
     ...
     *
     * The one field that does NOT survive is the autogenerated `id`: Room
     * reassigns it on insert, so a restored row comes back as 2, then 3, and so
     * on. Measured, not assumed - see tools/qa/phone/capture-collection.mjs,
     * which reads the row back off the device. Nothing depends on the old value:
     * the id is never persisted anywhere else, `removeFavorite` re-reads the
     * entity it is handed, and FavoritesAdapter's DiffCallback treating the
     * restored row as a new item is the right animation for a restore anyway.
     *
