/*
 * Generates repair-cta-size/code.js from main.tmpl.js by splicing in the
 * frozen classifier from the trial plugin.
 *
 *   node tools/figma-export/repair-cta-size/build.mjs [--check]
 *
 * The repair must judge roles with exactly the approved classifier, so the rule block
...
if (process.argv.includes("--check")) {
  ...
  if (current !== generated) {
    console.error("code.js is stale - re-run: node tools/figma-export/repair-cta-size/build.mjs");
    process.exit(1);
  }
